...
def create_bundle_posting(seller, creation=None, window_start=None, window_end=None):
    ...
    if creation == None:
        ...
        creation = fake.date_time_between(
            start_date="-6w", tzinfo=timezone.now().tzinfo
        )
...
def create_reservation(
    ...
    if selected_posting == None:
        postings = Bundle_posting.objects.annotate(
            num_reservations=Count("reservation")
        )
    ...
    if random.random() < 0.6:
        chosen_minutes = start_minutes + random.randint(
            0, (end_minutes - start_minutes) // 2
        )
    else:
...
def create_issue_report(type, consumer=None):
    ...
    creation_time = fake.date_time_between(
        start_date="-6w", end_date="now", tzinfo=timezone.now().tzinfo
    )
...
def run_seed(self, mode, seed):
    ...
    for consumer in consumers:
        for i in range(week_range):
            ...
            create_reservation(
                ...
                starting_date=start_week,
                ending_date=end_week,
            )
    ...
    create_bundle_posting(
        ...
        window_start=time(10, 00),
        window_end=time(23, 00),
    )
    create_bundle_posting(
